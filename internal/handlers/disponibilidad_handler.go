...
func ExisteCruceHorario(nueva models.DisponibilidadDocente) bool {

	for _, d := range storage.Disponibilidades {

		if d.DocenteID != nueva.DocenteID {
			continue
		}

		if d.DiaSemana != nueva.DiaSemana {
			continue
		}

		if nueva.HoraInicio < d.HoraFin &&
			nueva.HoraFin > d.HoraInicio {
...
func CreateDisponibilidad(w http.ResponseWriter, r *http.Request) {
	...
	if ExisteCruceHorario(disponibilidad) {
		http.Error(
			w,
